...
@if (session('info'))
    <x-ui.flash class="bg-yellow-100 border-yellow-300 p-3 border-b shadow text-center">
        <x-ui.icon name="information-circle" class="text-yellow-500" viewBox="0 2 20 20" /> {{ session('info') }}
    </x-ui.flash>
@endif

@if (session('error'))
    <x-ui.flash class="bg-red-300 border-red-500 p-3 border-b shadow text-center">
