...
<{{ $tag }} @if ($vueClick) @@click.prevent="{{$vueClick}}" @endif {{ $attributes->merge(['class' => 'bg-emerald-500 active:bg-emerald-600 active:border-emerald-700 py-1 px-6 text-white inline-block text-center rounded-md shadow border border-emerald-600 transition-colors']) }}>{{ $slot }}</{{ $tag }}>
